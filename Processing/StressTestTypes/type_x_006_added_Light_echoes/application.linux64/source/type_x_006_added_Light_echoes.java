...
public class type_x_006_added_Light_echoes extends PApplet {
...
public void setup() {
  ...
  qlc = new NetAddress("127.0.0.1", 7700);
  background(0);
}
...
public void draw() {
  ...
  if (masterswitch == 1) {
    ...
    if (mastercount == switchlen) {
      mastercount = 0;
      switchlen = PApplet.parseInt(random(1000));
      typeswitch = PApplet.parseInt(random(1, 5));

      //sending light trigger messages ONLY once
      if (typeswitch==1) {
        OscMessage qlcMsg = new OscMessage("/lights/vertical");
        qlcMsg.add(1);
        oscP5.send(qlcMsg, qlc);
      }
      if (typeswitch==2) {
        OscMessage qlcMsg = new OscMessage("/lights/boxes");
        qlcMsg.add(1);
        oscP5.send(qlcMsg, qlc);
      }
      if (typeswitch==3) {
        OscMessage qlcMsg = new OscMessage("/lights/horizontal");
        qlcMsg.add(1);
        oscP5.send(qlcMsg, qlc);
      }
       if (typeswitch==4) {
        OscMessage qlcMsg = new OscMessage("/lights/strobe");
        qlcMsg.add(1);
        oscP5.send(qlcMsg, qlc);
      }
    }
...
public void type1() {
  ...
  for (int x = 0; x < width; x+=largo) 
  {
    ...
    r += PApplet.parseInt(random(0, 63));
    g += PApplet.parseInt(random(0, 63));
    b += PApplet.parseInt(random(0, 63));
  ...
  if (count > 30) {
    ...
    largo = width/PApplet.parseInt(divisor);
    divisor = divisor*1.1f;
    if (divisor == 1) {
      divisor = 2;
    }
    ...
    if (prev != PApplet.parseInt(divisor)) {
      OscMessage verticalMsg = new OscMessage("/processing/verticallines");
      verticalMsg.add(PApplet.parseInt(divisor));
...
public void type2() {
  ...
  if (count2 == cyclen && flip == 0) {
    ...
    OscMessage blockMsgTotal = new OscMessage("/processing/blocks");
    blockMsgTotal.add(PApplet.parseInt(horizontal*vertical));
    //send the number of horizontal rows to SuperCollider to be dealt with
    OscMessage blockMsgHorizontal = new OscMessage("/processing/horizontalblocks");
    blockMsgHorizontal.add(PApplet.parseInt(horizontal));
    //send strobe information to Python at the same address but different port
    oscP5.send(blockMsgTotal, python);
    oscP5.send(blockMsgHorizontal, supercollider);
  }
...
public void type3() {
  int prev;
  prev = PApplet.parseInt(divisor3);
  ...
  if (count3 > 30) {
    //casting divisor as an int here to avoid math problems later on down the line
    largo3 = height/PApplet.parseInt(divisor3);
    divisor3 = divisor3*1.1f;
    if (divisor3 <= 1) {
      divisor3 = 2;
    }
    //scale this to the screen. Could do with this scrolling back a la arduino doublefade but we'll come back to that
    ...
    if (prev != PApplet.parseInt(divisor3)) {
      OscMessage horizontalMsg = new OscMessage("/processing/horizontallines");
      horizontalMsg.add(PApplet.parseInt(divisor3));
      //send strobe information to Python at the same address but different port
...
public void type4() {
  ...
  sr = map(PApplet.parseInt(random(63)), 0, 63, 0, 255);
  sg = map(PApplet.parseInt(random(63)), 0, 63, 0, 255);
  sb = map(PApplet.parseInt(random(63)), 0, 63, 0, 255);
  ...
  if (count4 >= 60 - len) {
    ...
    strobeMsg.add(PApplet.parseInt(sr));
    strobeMsg.add(PApplet.parseInt(sg));
    strobeMsg.add(PApplet.parseInt(sb));
    //send strobe information to Python at the same address but different port
...
public void strobeMsg() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "type_x_006_added_Light_echoes" };
    if (passedArgs != null) {
